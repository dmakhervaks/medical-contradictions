...
def get_evaluator(metric, samples, data_name, split, steps_in_epoch=0, pos_class=1, main_eval_loop=False, test_loop=False, batch_size=32, best_threshold_stats=None):
...
def create_tags(base_model_name, eval_data_info, fine_tuned_datasets):
...
def main(model, model_save_path, args, num_labels, eval_data_info):
    ...
    if args.eval:
        ...
        test_samples_evaluator = get_evaluator(eval_data_info['metric'],test_samples,eval_data_name,"test", pos_class=POS_CLASS, test_loop=True,
                                                batch_size=args.eval_batch_size, best_threshold_stats=best_threshold_stats) 
...
if __name__ == "__main__":
    ...
    tags = create_tags(base_model_name, eval_data_info, fine_tuned_datasets)
    ...
    main(model, model_save_path, args, num_labels, eval_data_info)
