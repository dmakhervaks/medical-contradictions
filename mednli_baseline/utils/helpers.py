...
def get_model_params(cfg, W_emb, num_labels):
    model_params = dict(
...
def get_dataset(cfg):
    cache_filename = cfg.cache_dir.joinpath(f'{cfg.dataset.value}_dataset_{int(cfg.lowercase)}_{cfg.max_len}.pkl')
    if not cache_filename.exists():
        mednli_train, mednli_dev, mednli_test = load_mednli(cfg)
        three_labels = True if 'mednli' in cfg.dataset.value else False
        dataset_train = NLIDataset(mednli_train, lowercase=cfg.lowercase, max_len=cfg.max_len, three_labels=three_labels)
        dataset_dev = NLIDataset(mednli_dev, vocab=dataset_train.vocab, lowercase=cfg.lowercase, max_len=cfg.max_len, three_labels=three_labels)
        dataset_test = NLIDataset(mednli_test, vocab=dataset_train.vocab, lowercase=cfg.lowercase, max_len=cfg.max_len, three_labels=three_labels)
